document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildPdfContent(cardsToInclude) {
        ...
        let dateInput = document.getElementById('date_exp');
        let rawDate = dateInput ? dateInput.value : todayStr;
        ...
        let contentHTML = `
            <div style="text-align: center; border-bottom: 2px solid #1c3d5a; padding-bottom: 15px; margin-bottom: 25px;">
                ...
                <p style="margin: 2px 0; font-size: 12px; color: #5f6368; font-weight: 500;">Division exploitation des ouvrages d'assainissement liquide</p>
                <p style="margin: 4px 0 0 0; font-size: 13px; color: #1c3d5a; font-weight: 800;">Service : STEP</p>
                <p style="margin: 8px 0 0 0; font-size: 13px; color: #d94f1c; font-weight: bold;">Date : ${formattedDate}</p>
            </div>
        ...
        allCards.forEach((card, index) => {
            ...
            let cardTitleSpan = card.querySelector('.header-title span');
            let cardTitle = cardTitleSpan ? cardTitleSpan.innerText : `Section ${cardNum}`;
            ...
            if (cardNum === 3) {
                ...
                blocks.forEach(block => {
                    ...
                    if((photos && photos.avant && photos.avant.length > 0) || (photos && photos.apres && photos.apres.length > 0)) {
                        ...
                        (photos.avant || []).forEach(p => { contentHTML += `<img src="${p}" style="width:170px; height:130px; object-fit:cover; border-radius:6px; border:1px solid #ccc;">`; });
                        contentHTML += `</div></div>`;
                        ...
                        (photos.apres || []).forEach(p => { contentHTML += `<img src="${p}" style="width:170px; height:130px; object-fit:cover; border-radius:6px; border:1px solid #ccc;">`; });
    ...
    if(btnGenerateAllPDF) {
        btnGenerateAllPDF.addEventListener('click', () => {
            ...
            let d = document.getElementById('date_exp')?.value || 'rapport';
    ...
    document.querySelectorAll('.btn-print-card').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            let d = document.getElementById('date_exp')?.value || 'section';
